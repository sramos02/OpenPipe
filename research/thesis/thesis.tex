...
Selenium is primarily used for the automated testing of web applications, and is ofter used in conjunction with a unit testng framework.  Selenium is however not limited to this set of tasks and can be extremely useful for other tasks such as performance testing and analysis. OpenPipe utilized Selenium to help clarify the performance gains and penalities when using the framework under differant usage scenarios and server load. Utilizing Selenium it became trivial to automate web page loading, and record large datasets of performance data.
...
Inserting JavaScript into an existing DOM document needs to be done in a slightly differant way than just appending the raw JavaScript data to the current HTML document being loaded. To do this in a reliable and cross browser way the boiler plate DOM function, 'createElement', is used to create a new script tag. Once the script tag is created the type and source code attributes are set indepentatly using the JSON data that is sent with a piplet's JavaScript component. The code that accomplishes the JavaScript insertion can be found in figure \ref{fig:javascriptInsertion}
...
To collect data a simple but powerful script [see figure \ref{fig:seleniumScript}] was developed utilizing the Selenium framework to automate the systematic retrieval of performance timing data for OpenPipe enabled web pages. The web page selected for data collection and analysis is provided in the set of example for OpenPipe. The URL  installed on a local system is  'HTTP://localhost/dev/openpipe/example/codeigniter/2.1.0/index.php/fakebook'. Selenium was a perfect candidate for automated retrieval of data, because unlike other types of web request tools such as cURL or Wget which just send and receive raw HTTP data, Selenium drives a physical web browser through the utilizing of underlying browser API systems. The means that data is loaded and processed exactly the same as when an end user accesses a given website. This includes web requests, web reponse,  loading and unloading of the DOM,  and JavaScript execution. JavaScript execution was a crucial component in determining the load time for an OpenPipe web page, since all data is unpacked and loaded in browser via JavaScript after it has been received from the server.
...
\item \textbf{Type} - Represents the type of external event that the server is issuing to complete the given web page request. Three types of web page requests were simulated:
	...
	\end{enumerate}
\item \textbf{Load} - The ammount of load that is being put on the web server during the testing time. Load was simulated using the command line tool named, 'Siege'. The number presented in this column represents the total number of concurrent connection being issues from Siege during the tests.
\item \textbf{Response} - The initial response time for a non piped page. \begin{math} responseStart - requestStart \end{math} 
\item \textbf{Response Piped} - The initial response time for a non piped page. This is calculated by recording the load time of the first piece of content within a pipelet (users sees something) and comparing that to the request start time. \begin{math} firstPipeletLoadTimeEnd - requestStart \end{math} 
\item \textbf{Total Time} and \textbf{Total Time Piped}   - The total time it took to the load the page, starting at request start and ending at the final DOM loaded event. \begin{math} loadEventEnd - requestStart \end{math} 
\end{enumerate}


\begin{figure}[H]
\caption{Calculated response and load time in milliseconds. Data is based on timing data collected from automated browser runs via Selenium}
...
\emph{Web Service}		& 10				& 5053.040816	& 767.5510204   			& 5110.591837		& 5110.591837    			\\
\hline
\emph{Plain HTML}		& 25						& 100						& 200    & 100						& 200    \\
\emph{Database}		& 25						& 100						& 200    & 100						& 200    \\
\emph{Web Service}		& 25						& 100						& 200    & 100						& 200    \\
\hline
\emph{Plain HTML}		& 50						& 100						& 200    & 100						& 200    \\
\emph{Database}		& 50						& 100						& 200    & 100						& 200    \\
\emph{Web Service}		& 50						& 100						& 200    & 100						& 200    \\
\hline
\emph{Plain HTML}		& 100					& 100						& 200    & 100						& 200    \\
\emph{Database}		& 100					& 100						& 200    & 100						& 200    \\
\emph{Web Service}		& 100					& 100						& 200    & 100						& 200    \\
\end{tabular}
\end{figure}
...
Available: http://dvcs.w3.org/hg/webperf/raw-file/tip/specs/NavigationTiming/Overview.html

\bibitem{siegeHomepage}
Siege Home,  January 30, 2012 [Online].
Available: http://www.joedog.org/siege-home/
